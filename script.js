...
// Carrossel de fotos da cobertura do Nordestão 2026 — autoplay, setas, dots e swipe
(function () {
  const carousel = document.getElementById('coberturaCarousel');
  if (!carousel) return;

  const track    = document.getElementById('coberturaTrack');
  const dotsWrap = document.getElementById('coberturaDots');
  const prevBtn  = carousel.querySelector('.cobertura-prev');
  const nextBtn  = carousel.querySelector('.cobertura-next');
  const slides   = track.children;
  const total    = slides.length;
  const AUTOPLAY_MS = 5000;
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  for (let i = 0; i < total; i++) {
    const dot = document.createElement('button');
    dot.type = 'button';
    dot.className = 'cobertura-dot' + (i === 0 ? ' is-active' : '');
    dot.setAttribute('role', 'tab');
    dot.setAttribute('aria-label', 'Foto ' + (i + 1));
    dot.setAttribute('aria-selected', i === 0 ? 'true' : 'false');
    dot.addEventListener('click', () => goTo(i, true));
